...
`collection_commands` now stores downloader `profile` and `options` as explicit fields alongside `params`. New command writes no longer rely on reserved keys inside `params`. During orchestration DataHub passes `profile`, clean `params`, `options`, and selected `scope_items` to downloader `/sync`.

DataHub still owns command batches, canonical scope selection, ingestion, checkpoints, processing, and Domain API. It does not manage DCP account credentials and does not read downloader-local Envelope files.
